    ...
    TreeDepth(0)
{
	...
	root->NumChildren = Params.MultiAgent && !Params.JointQActions[i] ? Simulator.GetNumAgentActions() : 
			Simulator.GetNumActions();
	root = ExpandNode(Simulator.CreateStartState(), Params.MultiAgent ? i+1 : i);
    ...
    for (int i = 0; i < Simulator.GetNumAgents(); i++)
    {
	if (Params.RewardAdaptive[i] && Params.MultiAgent)
	{
		...
		rewardTemplate->RewardWeight = Rollout(*state, i+1);
...
{
    int action = GreedyUCB(vnode, true, index);
