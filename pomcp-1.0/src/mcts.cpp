...
double MCTS::SimulateQ(STATE& state, QNODE& qnode, int action, const int& index)
{
    ...
    {
	if (index == 1)
	    action = action + Simulator.GetNumAgentActions()*Random(Simulator.GetNumAgentActions());
	else
	    action = Random(Simulator.GetNumAgentActions()) + Simulator.GetNumAgentActions()*action;
    }
