    ...
    TreeDepth(0)
{
	...
	//other agent rootValue
	VNODE* othervalue;
	if (Params.RewardAdaptive[i])
	{
	    othervalue->NumChildren = Params.MultiAgent && !Params.JointQActions[i] ? Simulator.GetNumAgentActions() : 
			    Simulator.GetNumActions();
	    othervalue = ExpandNode(state, Params.MultiAgent ? i+1 : i, i == 0 ? 2 : 1);
	}
	...
	if (Params.RewardAdaptive[i])
	{
		...
		Statuses[i].RewardParams = rewardTemplate->RewardParams;
		STATE* state = Roots[i]->Beliefs().CreateSample(Simulator);
		double otherReward = 0;
		rewardTemplate->RewardWeight = Rollout(*state, i+1, otherReward);
		Roots[i]->Beliefs().AddRewardSample(rewardTemplate);
	    }
	}
    }
...
bool MCTS::Update(int action, int observation, double reward, const int& index)
{
	...
	VNODE::Free(OtherValues[index == 0 ? index : index-1], Simulator);
	VNODE* newOtherValue = ExpandNode(state, index, index == 2 ? 1 : 2);
	if (!Params.MultiAgent)
	    OtherValues[index] = newOtherValue;
	else
	    OtherValues[index-1] = newOtherValue;
    }
...
void MCTS::RolloutSearch(const int& index)
{
    ...
    for (int i = 0; i < Params.NumSimulations; i++)
    {
		...
		Statuses[index == 0 ? index : index-1].RewardParams = rewardTemplate->RewardParams;
	    }
		...
		OtherValues[index == 0 ? index : index-1]->Child(othertreeaction).Value.Add(otherTotalReward);
...
void MCTS::UCTSearch(const int& index)
{
	...
	double otherTotalReward = 0.0;
        double totalReward = SimulateV(*state, Roots[index == 0 ? index : index-1], index, 
					otherTotalReward, OtherValues[index == 0 ? index : index-1]);
...
double MCTS::SimulateV(STATE& state, VNODE* vnode, const int& index, double otherTotalReward, VNODE* otherValue)
{
    
    int action = GreedyUCB(vnode, otherValue, true, index); //for reward adaptive, should return joint action!
	...
	QNODE& otherQ = otherValue->Child(otheraction);
	totalReward = SimulateQ(state, qnode, ownaction, index, otherTotalReward, otherQ);
    }
    else
    {
	...
	if (Params.RewardAdaptive[index == 0 ? index : index-1])
	{
	    QNODE& otherQ = otherValue->Child(action);
	    totalReward = SimulateQ(state, qnode, action, index, otherTotalReward, otherQ);
	}
	else
	    totalReward = SimulateQ(state, qnode, action, index, otherTotalReward, qnode);
    }
    ...
    if (Params.RewardAdaptive[index == 0 ? index : index-1])
    {
	otherValue->Value.Add(otherTotalReward);
	//rave not compatible
    }
...
double MCTS::SimulateQ(STATE& state, QNODE& qnode, int action, const int& index, double otherTotalReward, QNODE& otherQ)
{
    int observation;
	...
	if (Params.RewardAdaptive[index == 0 ? index : index-1])
	    Simulator.UpdateAlpha(otherQ, state);
    }
    bool terminal = Simulator.Step(state, action, observation, immediateReward, Statuses[index == 0 ? index : index-1]);
	...
	if (!terminal)
	{
	    TreeDepth++;
	    if (vnode)
		delayedReward = SimulateV(state, vnode, index, otherDelayedReward, otherV);
	    else
		delayedReward = Rollout(state, index, otherDelayedReward);
	    TreeDepth--;
	}
    ...
    {
	VNODE*& otherV = otherQ.Child(Simulator.GetAgentObservation(observation,index));
	
	if (!otherV && otherQ.Value.GetCount() >= Params.ExpandCount)
	    otherV = ExpandNode(&state, index, index == 2 ? 1 : 2);
	...
	otherQ.Value.Add(otherTotalReward);
    }
    return totalReward;
}
...
int MCTS::GreedyUCB(VNODE* vnode, VNODE* otherValue, bool ucb, const int& index) const
{
