...
MCTS::~MCTS()
{
	...
	VNODE::Free(OriginalRoots[i], Simulator);
    }
    VNODE::FreeAll();
}
...
void MCTS::ResetRoot(const int& index)
{
    if (!Params.MultiAgent)
	Roots[index] = OriginalRoots.at(index);
    else
	Roots[index-1] = OriginalRoots.at(index-1);
}
