...
void EXPERIMENT::Run()
{
	...
	if (statusComm)
	{
	    ...
	    status.MessagesToBeSent.push_back(Real.SelectMessage(mcts.GetStatus(1), mcts.GetHistory(1)));
	    status.MessagesToBeSent.push_back(Real.SelectMessage(mcts.GetStatus(2), mcts.GetHistory(2)));
	}
