...
void EXPERIMENT::Run()
{
    ...
    double discount = 1.0;
    bool terminal = false;
    bool outOfParticles = false, outOfParticles2 = true;
    int t;
	...
	if (!SearchParams.MultiAgent)
	    ...
	    action = Simulator.GetNumAgentActions()*Simulator.GetAgentAction(mcts.SelectAction(1),1) + 
			Simulator.GetAgentAction(mcts.SelectAction(2),2);
	...
	outOfParticles = !mcts.Update(action, observation, reward, 1);
	if (SearchParams.MultiAgent)
	    outOfParticles2 = !mcts.Update(action, observation, reward, 2);
	...
	if (SearchParams.MultiAgent)
		...
		action = Simulator.GetNumAgentActions()*Simulator.SelectRandom(*state, history, mcts.GetStatus(1), 1)
