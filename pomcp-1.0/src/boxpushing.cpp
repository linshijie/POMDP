...
STATE* BOXPUSHING::CreateStartState() const
{
	...
	do
	{
	    ...
	    pe.Position = COORD(Random(XSize), 0);
	    pe.Length = 1;
	}
	while (Collision(*bpstate, pe));
    ...
    int& observation, double& reward) const
{
    ...
    observation = NumObservations*GetAgentObservation(bpstate, 0) + GetAgentObservation(bpstate, 1);
    
    return bpstate.NumBoxesRemaining == 0;
}
    ...
    int stepObs, const STATUS& status) const
{
    ...
    int simObs = NumObservations*GetAgentObservation(bpstate, 0) + GetAgentObservation(bpstate, 1);
...
{
    for (int a = 0; a < NumActions*NumActions; a++)
        legal.push_back(a);
}
        ...
        std::vector<int>& actions, const STATUS& status) const
{
    ...
    int lastObs0 = lastObs/NumActions;
    int lastObs1 = lastObs%NumActions;
...
{
    int observation0 = observation/NumObservations;
    int observation1 = observation%NumObservations;
    
    cout << observation0 << ", " << observation1 << "\n";
}

void BOXPUSHING::DisplayAction(int action, std::ostream& ostr) const
{
    int action0 = action/NumActions;
    int action1 = action%NumActions;
    
    cout << action0 << ", " << action1 << "\n";
}