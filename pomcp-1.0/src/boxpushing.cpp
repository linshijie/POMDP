        ...
        std::vector<int>& actions, const STATUS& status) const
{
    ...
    GeneratePreferredAgent(lastObs0, actions0);
    //GenerateLegalAgent(actions1);
    GeneratePreferredAgent(lastObs1, actions1);
    
    for (int i = 0; i < (int)actions0.size(); i++)
	for (int j = 0; j < (int)actions1.size(); j++)
	    actions.push_back(NumAgentActions*actions0[i] + actions1[j]);
}

void BOXPUSHING::GenerateLegalAgent(std::vector<int>& actions) const
{
    for (int a = 0; a < NumAgentActions; a++)
        actions.push_back(a);
}

void BOXPUSHING::GeneratePreferredAgent(const int& lastObs, std::vector<int>& actions) const
{
    actions.clear();
    switch(lastObs)
    {
