...
{
    BOXPUSHING_STATE& bpstate = safe_cast<BOXPUSHING_STATE&>(state);
    int div = NumAgentActions;
    if (status.UseCommunication)
	div *= NumAgentMessages;
    
    int actions[] = {action%div, action/div};
    int messages[] = {0, 0};
	...
	//std::cout << "actions " << action0<< " " << action1 << "\n";
	//std::cout << "messages " << messages[0]<< " " << messages[1] << "\n";
	for (int i = 0; i < NumAgents; i++)
    ...
    int obs0 = MakeAgentObservation(bpstate, 0);
    int obs1 = MakeAgentObservation(bpstate, 1);
    ...
    int n = NumAgentObservations;
    if (status.UseCommunication)
	n *= NumAgentMessages;
    
    observation = obs0 + n*obs1;
...
int BOXPUSHING::MakeAgentObservation(const BOXPUSHING_STATE& bpstate, const int& agentindex) const
{
...
bool BOXPUSHING::IsActionMultiagent(const int& action, const HISTORY& history, const bool& comm) const
{
    ...
    {
	int lastCombinedObs = history.Back().Observation;
	int lastObs = GetObservationComponentFromObservation(lastCombinedObs);
	int lastMes = GetMessageComponentFromObservation(lastCombinedObs);
	int actionComp = GetActionComponentFromAction(action);
	int messComp = GetMessageComponentFromAction(action);
	if (actionComp == MOVE && lastObs == LARGE_BOX_OBS && lastMes == LARGE_BOX_MES)
	    return true;
	return false;
    }
        ...
        std::vector<int>& legal, const STATUS& status) const
{
	...
	GenerateLegalAgent(state, history, actions0, status, status.perspindex);
	for (int i=0; i<(int)actions0.size(); i++)
	    for (int j=0; j<(int)actions0.size(); j++)
	    {
		//if (status.perspindex == 0)
		    legal.push_back(actions0[i] + NumAgentActions*NumAgentMessages*actions0[j]);
		//else
		 //   legal.push_back(j + NumAgentActions*NumAgentMessages*actions0[i]);
	    }
    }
}
    ...
    std::vector<int>& actions, const STATUS& status, const int& index) const
{
	...
	switch(lastObs)
	{
	    ...
	    case(WALL_OBS):
	    case(AGENT_OBS):
		actions.push_back(STAY + NumAgentActions*NO_MES);
		actions.push_back(TURN_CW + NumAgentActions*NO_MES);
		actions.push_back(TURN_CCW + NumAgentActions*NO_MES);
		actions.push_back(MOVE + NumAgentActions*NO_MES);
		break;
	    case(SMALL_BOX_OBS):
		//if (lastMes != LARGE_BOX_MES)
		    actions.push_back(MOVE + NumAgentActions*SMALL_BOX_MES);
		//else{
		actions.push_back(TURN_CW + NumAgentActions*SMALL_BOX_MES);
		actions.push_back(TURN_CCW + NumAgentActions*SMALL_BOX_MES);
		//}
		actions.push_back(STAY + NumAgentActions*SMALL_BOX_MES);
		break;
	    case(LARGE_BOX_OBS):
		//if (lastMes == LARGE_BOX_MES)
		    actions.push_back(MOVE + NumAgentActions*LARGE_BOX_MES);
		//else
		//{
		actions.push_back(STAY + NumAgentActions*LARGE_BOX_MES);
		actions.push_back(TURN_CW + NumAgentActions*LARGE_BOX_MES);
		actions.push_back(TURN_CCW + NumAgentActions*LARGE_BOX_MES);
		//}
		break;
	    case(LARGE_BOX_AGENT_OBS):
		actions.push_back(MOVE + NumAgentActions*LARGE_BOX_MES);
		if (!status.HumanDefined)
		{
		    actions.push_back(STAY + NumAgentActions*LARGE_BOX_MES);
		    actions.push_back(TURN_CW + NumAgentActions*LARGE_BOX_MES);
		    actions.push_back(TURN_CCW + NumAgentActions*LARGE_BOX_MES);
...
void BOXPUSHING::DisplayObservation(const STATE& state, int observation, ostream& ostr) const
{
    ...
    int observation0 = GetAgentObservation(observation,1,NumAgentMessages);
    int observation1 = GetAgentObservation(observation,2,NumAgentMessages);
...
void BOXPUSHING::DisplayAction(int action, std::ostream& ostr) const
{
    ...
    int action0 = GetAgentAction(action, 1, NumAgentMessages);
    int action1 = GetAgentAction(action, 2, NumAgentMessages);
