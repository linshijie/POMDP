...
public:
    ...
    int GetAgentAction(const int& action, const int& index, int commFactor) const { return index > 1 ? action/(NumAgentActions*commFactor) : 
									    action%(NumAgentActions*commFactor);}
    int GetAgentObservation(const int& observation, const int& index, int commFactor) const { return index > 1 ? 
						    observation/(NumAgentObservations*commFactor) : observation%(NumAgentObservations*commFactor);}
						   
    int GetActionComponentFromAction(const int& action) const { return action%NumAgentActions; }
    int GetMessageComponentFromAction(const int& action) const { return action/NumAgentActions; }
    int GetObservationComponentFromObservation(const int& observation) const { return observation%NumAgentObservations; }
    int GetMessageComponentFromObservation(const int& observation) const { return observation/NumAgentObservations; }
