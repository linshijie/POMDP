    ...
    TreeDepth(0)
{
	...
	status.UseCommunication = Params.UsesComm[i];
	...
	QNODE::NumMessageValues = Simulator.GetNumAgentMessages();
...
bool MCTS::Update(int action, int observation, double reward, const int& index)
{
    ...
    //if (Statuses[index == 0 ? index : index-1].UseCommunication)
	//observation += Simulator.GetNumAgentObservations()*Statuses[index > 0 ? index-1 : index].LastMessageReceived;
...
void MCTS::UCTSearch(const int& index)
{
	...
	Statuses[index == 0 ? index : index-1].MainMessageValueSequence.clear();
	    ...
	    //!Statuses[index == 0 ? index : index-1].TerminalReached)
	{
		...
		Statuses[index == 0 ? index : index-1].LearnMessageValueSequence.clear();
		...
		//if (tempTotalReward > totalReward && tempTotalReward > 0)
		{
			...
			for (int j = 0; j < size; j += 2)
			{
				...
				//std::pair<int,double> mp = Statuses[index == 0 ? index : 
				    //index-1].MainMessageValueSequence[indQ];
				//qnode.MessageValues[mp.first].Subtract(mp.second);
				indQ++;
			    }
			    if (j+1 < size)
			    {
		    ...
		    //Statuses[index == 0 ? index : index-1].MainMessageValueSequence = 
			    //Statuses[index == 0 ? index : index-1].LearnMessageValueSequence;
		}
		else
		{
		    doLearn = false;
			...
			for (int j = 0; j < size; j += 2)
			{
				...
				qnode.OtherAgentValues[0].Subtract(Statuses[index == 0 ? index : 
				    index-1].LearnOtherQValueSequence[indQ]);
				//std::pair<int,double> mp = Statuses[index == 0 ? index : 
				//    index-1].LearnMessageValueSequence[indQ];
				//qnode.MessageValues[mp.first].Subtract(mp.second);
				indQ++;
			    }
			    if (j+1 < size)
...
double MCTS::SimulateQ(STATE& state, QNODE& qnode, int action, const int& index, double otherTotalReward)
{
    ...
    int origAction = action;
    
    
    if (Params.MultiAgent && !Params.JointQActions[index == 0 ? index : index-1])
    {
	int otherAction = index == 1 ? Simulator.SelectRandom(state, GetHistory(index), GetStatus(index), 2) : 
		Simulator.SelectRandom(state, GetHistory(index), GetStatus(index),1);
	if (Statuses[index == 0 ? index : index-1].UseCommunication && !qnode.MessageActions.empty())
	{
	    /*int bestA = 0;
	    double bestQ = qnode.MessageValues[0].GetCount();
	    for (int j = 1; j < Simulator.GetNumAgentMessages(); j++)
	    {
		double currQ = qnode.MessageValues[j].GetCount();
		if (currQ > bestQ)
		{
		    bestQ = currQ;
		    bestA = j;
	    ...
	    otherAction = qnode.SelectRandomMessageAction();
	}
	if (index == 1)
	    action = action + 
		Simulator.GetNumAgentActions()*otherAction;
	...
	if (index == 1)
	{
	    int msg = Simulator.SelectMessage(GetStatus(index), GetHistory(index), origAction);
	    Statuses[index == 0 ? index : index-1].MessagesToBeSent.push_back(msg);
	    lastSent = msg;
	}
	else
	    Statuses[index == 0 ? index : index-1].MessagesToBeSent.push_back(Simulator.GetAgentAction(action,2));
	if (index == 2)
	{
	    int msg = Simulator.SelectMessage(GetStatus(index), GetHistory(index), origAction);
	    Statuses[index == 0 ? index : index-1].MessagesToBeSent.push_back(msg);
	    ...
	    Statuses[index == 0 ? index : index-1].MessagesToBeSent.push_back(Simulator.GetAgentAction(action,1));
    ...
    Histories[index == 0 ? index : index-1].Add(index == 0 || Params.JointQActions[index-1] ? action : Simulator.GetAgentAction(action, index), 
			       index == 0 ? observation : Simulator.GetAgentObservation(observation, index));
    ...
    int childIndex = observation;
    if (index > 0)
    {
	//if (!Statuses[index == 0 ? index : index-1].UseCommunication)
	    childIndex = Simulator.GetAgentObservation(observation, index);
	//else
	    //childIndex = Simulator.GetAgentObservation(observation, index) + Simulator.GetNumAgentObservations()*Statuses[index > 0 ? index-1 : index].LastMessageReceived;
    }
    
    
    VNODE*& vnode = qnode.Child(childIndex);//index == 0 ? qnode.Child(observation) : 
	...
	if (Statuses[index == 0 ? index : index-1].LearningPhase)
	{
	    Statuses[index == 0 ? index : index-1].LearnSequence.push_back(childIndex);
	    Statuses[index == 0 ? index : index-1].LearnFullSequence.push_back(childIndex);
	}
	else
	{
	    Statuses[index == 0 ? index : index-1].MainSequence.push_back(childIndex); 
	    Statuses[index == 0 ? index : index-1].MainFullSequence.push_back(childIndex); 
	}
    }
	...
	if (Statuses[index == 0 ? index : index-1].UseCommunication)
	{
	    int lastMessage = Statuses[index > 0 ? index-1 : index].LastMessageReceived;
	    if (lastMessage < 0 || lastMessage >= Simulator.GetNumAgentMessages())
		lastMessage = 0;
	    qnode.MessageValues[lastMessage].Add(totalReward);
	    double newVal = qnode.MessageValues[lastMessage].GetValue();
	    if (!qnode.MaxMessageSet)
	    {
		qnode.MaxMessageValue = newVal;
		qnode.MaxMessageSet = true;
	    }
	    else if (newVal > qnode.MaxMessageValue)
		qnode.MaxMessageValue = newVal;
	    
	    if (Statuses[index == 0 ? index : index-1].LearningPhase)
		Statuses[index == 0 ? index : index-1].LearnMessageValueSequence.push_back(std::make_pair(lastMessage,totalReward));
	    else
		Statuses[index == 0 ? index : index-1].MainMessageValueSequence.push_back(std::make_pair(lastMessage,totalReward));
	}
    }
...
int MCTS::GreedyUCB(VNODE* vnode, bool ucb, const int& index) const
{
	...
	for (int i = 0; i < maxIter2; i++)
	{
		...
		//else
		{
		    ...
		    //if (ucb)
			//maxMesQ += FastUCB(N, qnode.MessageValues[0].GetCount(), logN);
		    for (int j = 1; j < Simulator.GetNumAgentMessages(); j++)
		    {
			double mesQ = qnode.MessageValues[j].GetValue();
			//if (ucb)
			    //mesQ += FastUCB(N, qnode.MessageValues[j].GetCount(), logN);
			if (mesQ > maxMesQ)
			{
			    maxMesQ = mesQ;
			}
...
double MCTS::Rollout(STATE& state, const int& index, double otherReward)
{
	...
	/*if (Statuses[index == 0 ? index : index-1].UseCommunication)
	{
	    ...
	    if (index == 1)
	    {
		int msg = Simulator.SelectMessage(GetStatus(index), GetHistory(index), Simulator.GetAgentAction(action,index));
		Statuses[index == 0 ? index : index-1].MessagesToBeSent.push_back(msg);
		lastSent = msg;
	    }
	    else
	...
	bool useComm = Statuses[index == 0 ? index : index-1].UseCommunication;
	Statuses[index == 0 ? index : index-1].UseCommunication = false;
        terminal = Simulator.Step(state, action, observation, reward, Statuses[index == 0 ? index : index-1]);
	Statuses[index == 0 ? index : index-1].UseCommunication = useComm;
	//if (Statuses[index == 0 ? index : index-1].UseCommunication)
	   // Statuses[index > 0 ? index-1 : index].LastMessageReceived = Statuses[index > 0 ? index-1 : index].MessagesReceived[index-1]; 
...
STATE* MCTS::CreateTransform(const int& index)
{
    int stepObs;
    double stepReward;
